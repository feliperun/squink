...
pub fn info(comptime fmt: []const u8, args: anytype) void {
    write(if (json) std.Io.File.stderr() else std.Io.File.stdout(), fmt, args);
}

pub fn warn(comptime fmt: []const u8, args: anytype) void {
    write(std.Io.File.stderr(), "warning: " ++ fmt ++ "\n", args);
}
...
pub fn fail(comptime fmt: []const u8, args: anytype) void {
    ...
    last_error = msg;
    if (!json) write(std.Io.File.stderr(), "ERROR: {s}\n", .{msg});
}

/// Writes `value` as one line of JSON on stdout.
pub fn emit(value: anytype) void {
    emitted = true;
    write(std.Io.File.stdout(), "{f}\n", .{std.json.fmt(value, .{})});
}

pub fn write(f: std.Io.File, comptime fmt: []const u8, args: anytype) void {
    var buf: [4096]u8 = undefined;
    var w = f.writerStreaming(sys.io, &buf);
